...
def consume_item(item,t_inventory):
    if check_item(item,t_inventory):
        if t_inventory[item]<=0:
            print("선택하신 아이템의 수량이 부족하여 사용할수 없습니다.")
        else:
            t_inventory[item]-=1
            print("사용하신" + item + "수량은"+ str(t_inventory[item])+"입니다.")
    else:
        print(item + "이 존재하지 않습니다.")
...
def use_item(inventory) :
    while True:
        ...
        if option == 0:
            ...
            c_item = input("사용하실 아이템을 입력해 주세요.)")
...
while True:
    ...
    if option == 0:
        ...
        temp_name = input("선택할 캐릭터의 이름을 입력하시오.)")
